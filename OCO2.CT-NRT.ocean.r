...
ctnrt.ocean<-function(ident=ident, foot=foot.ocean, storeTF=FALSE, res=lon.res, ncdfpath=NULL){
...
recp.yr<-as.numeric(substr(ident,1,4));recp.mon<-as.numeric(substr(ident,6,7));recp.day<-as.numeric(substr(ident,9,10));recp.hr<-as.numeric(substr(ident,12,13))
...
if(unique(back.times[,"hr"]%%3!=0)){
  ...
  cat("oco2.ctnrt.ocean(): foot hours differ from CT hours, NO CT-NRT files found...\n")
