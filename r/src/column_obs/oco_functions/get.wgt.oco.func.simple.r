...
#!!! ignore q to deal with NA RH from HRRRv1, DW, 07/03/2024
get.wgt.oco.func.simple = function(output, oco.path, oco.fn = NA) {
	...
	if (is.null(oco.info)) {
		warning('get.wgt.oco.func.simple(): NO OCO info found for this receptor...please check\n')
		return()
	} # end if is.null
	...
	if (is.na(oco.zsfc)) stop('get.wgt.oco.func.simple(): satellite-based surface height is NA, please check...\n')
	...
	sel.p = sel.p %>% 
			...
			mutate(xpres = oco.psfc * exp(a.oco * (-xhgt)), 
                  ...
                  # interpolate AK_norm, apriori per parcel using its press info
    ...
    # modification to ignore q by DW, July 3, 2024
    if (!is.null(qt.prof)) {
        qt.bin = qt.prof %>% 
        ...
        if ( length(qt.bin$sphu) > 0 ) {
            cat('get.wgt.oco.func.simple(): ***warning: missing RH from met fields, ignoring q...only a very rough estimate for the dry-air subcolumn...\n')
            sel.p = sel.p %>% mutate(q = approx(qt.bin$xpres, qt.bin$sphu, 
                                                xout = xpres, rule = 2)$y)
        } else sel.p = sel.p %>% mutate(q = 0)
    } else sel.p = sel.p %>% mutate(q = 0)      # simply treat q as zero if NA

    sel.p = sel.p %>% arrange(desc(xpres))
	...
	cat('get.wgt.oco.func.simple(): Done computing PWF...\n')
