...
get.xdry.mod = function(output) {
	...
	nls = stats::nls(p.pres ~ p.psfc * exp(a * (-p.zagl)), 
					start = list(a = 1E-4))		
	...
	# -------------------------------------------------------------------------
	# use modeled sfc pres to calc corresponding pressure of release heights
	...
	qt.bin = qt.prof %>% 
			 mutate(xpres = pres.bound[findInterval(pres, pres.bound) + 1]) %>% 
			 group_by(xpres) %>% summarise_all(mean) %>% ungroup() %>% 
			 dplyr::select(-pres) %>% na.omit()

	# interpolate specific humidity 
	sel.p = sel.p %>% 
		    mutate(q = approx(x = qt.bin$xpres, y = qt.bin$sphu, xout = xpres, 
	...
	qt.entire = qt.prof %>% 
				mutate(xpres = entire.pres[findInterval(pres, entire.pres) + 1]) %>% 
				group_by(xpres) %>% summarise_all(mean) %>% 
				ungroup() %>% na.omit()	

	# get modeled dry-air column density for the entire column and for each particle
	sel.p$xdry.tot = sum((1 - qt.entire$sphu) / g / Mdry * entire.dp * 100)
	return(sel.p)
}
...
get.wgt.mod.func = function(output){
	...
	g = 9.8        	      	# m s-2
	Mdry = 29 / 1E3        	# kg mol-1
	sel.p = get.xdry.mod(output) %>% 
			mutate(xdry = (1 - q) / g / Mdry * dp * 100, pwf = xdry / xdry.tot)
	
	# all levels are modeled levels, since we removed the dependence from satellites, 
	# no apriori, AK_norm is simply 1. 
	xstilt.prof = sel.p %>% 
				  mutate(ak.norm = 1, ak.pwf = ak.norm * pwf, stiltTF = T) %>% 
