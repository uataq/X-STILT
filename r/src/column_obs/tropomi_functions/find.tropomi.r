...
find.tropomi = function(tropomi_path, timestr, lon_lat, bufferTF = F){
    ...
    # there could be alternative TROPOMI version from SRON with diff filename
    # readme https://ftp.sron.nl/open-access-data-2/TROPOMI/tropomi/ch4/SRON_CH4_product_Jan2022.pdf
    # fix file searching, DW, 11/17/2023 --------------------
    fns = list.files(tropomi_path, '.nc', full.names = T)
    sron_ch4TF = TRUE %in% unique(grepl('s5p_l2_ch4', fns))
    if ( sron_ch4TF ) {
        tropomi_str = paste(substr(timestr, 1, 4), substr(timestr, 5, 6), 
                            substr(timestr, 7, 8), sep = '_')
    } else tropomi_str = paste0('___', YYYYMMDD)
    fns = fns[grepl(tropomi_str, fns)]

    if (length(fns) == 0) { 
        cat('find.tropomi(): NO TROPOMI files found for the desired days...please check..\n')
        return() 
    }
    
    # if hour and beyond is given, further select some of the TROPOMI files
    # DW, 08/19/2021
    if ( !is.na(HH) & !sron_ch4TF ) {
        ...
        if (unique(HH_fns$V5) == 'CO' | TRUE %in% grepl('PAL', fns)) {
            HH_fns = HH_fns %>% dplyr::select(mn = V10, mx = V11)
        } else if (unique(HH_fns$V5) == 'HCHO') {
            HH_fns = HH_fns %>% dplyr::select(mn = V8, mx = V9)
        ...
        exact_indx = unique(c(which(datestr >= HH_fns$date_mn &
    ...
    for (f in 1 : length(fns)) {
        ...
        fn = fns[f]
        if ( grepl('s5p_l2_ch4', fn) ) {
            
            sel_df = load.tropomi.ch4.day(fn, lon_lat) %>% filter(!is.na(xch4)) 
            if (nrow(sel_df) > 0) {
                overpass.timestr = unique(substr(sel_df$time_utc, 1, 14))
                tmp_df = data.frame(overpass.start.time = min(overpass.timestr),
                                    overpass.end.time = max(overpass.timestr), 
                                    tot.count = nrow(sel_df), 
                                    qa0p5.count = nrow(sel_df %>% 
                                                       filter(qa >= 0.5)),
                                    qa0p4.count = nrow(sel_df %>% 
                                                       filter(qa >= 0.4)),
                                    qa0p7.count = nrow(sel_df %>% 
                                                       filter(qa >= 0.7)),
            ...
            if ('' %in% time) {
                ...
                ref_time = ncvar_get(dat, 'PRODUCT/time') # ref time in sec
                
                # offset from reference start time of measurement
                # milliseconds since @param time, [ground_pix, scaline]
                dt   = ncvar_get(dat, 'PRODUCT/delta_time') * 1E-3      # second
            ...
            # total vertical column of sulfur dioxide for the polluted scenario derived from the total slant column
            # Recommend to ignore data with qa_value < 0.5
            if (unique(grepl('_SO2_', fn)))     # mol m-2
                val = ncvar_get(dat, 'PRODUCT/sulfurdioxide_total_vertical_column')
            ...
            var_df = dcast(melt(var_array), Var1 + Var2~Var3, 
                           value.var = 'value')
            colnames(var_df)[1:2] = c('indx_across_track', 'indx_along_track')

            # qa_value > 0.75 is the recommended pixel filter. 
            ...
            if (nrow(sel_df) > 0) { # if found one file, return value
                cat(paste0('found the data that match the criteria; see nc file: ', basename(fn), '\n'))
                fn_df = strsplit.to.df(basename(fn)) 

                if ( unique(grepl('_CO_', fn)) | grepl('_PAL_', fn) ) { 
                    start.time = fn_df$V10
                    end.time = fn_df$V11 
                } else if (unique(grepl('_NO2_', fn)) | 
                           unique(grepl('_SO2_', fn)) | 
                           unique(grepl('_CH4_', fn)) ) { 
                    start.time = fn_df$V9
                    end.time = fn_df$V10 
                ...
                tmp_df = data.frame(start.time = start.time, 
                                    end.time = end.time, 
                ...
                # DW, 06/02/2021
                if (tmp_df$qa0p4.count == 0) {
                    cat('find.tropomi(): NO qualified data with QA >= 0.4 found')
                    next
                }
            }   # end if 
        }  # end if
