...
get.wgt.funcv6 <- function(output, oco.info, ak.wgt = T, pwf.wgt = T){
    ...
    # calculate the fs in Odell et al., 2012 for getting PWF for the bottom level
    # OCO pwf for the bottom level is usually half of that for its one level above
    # DW, 07/07/2020
    fs <- as.numeric((oco.psfc - oco.pres[2]) / (oco.pres[1] - oco.pres[2]))
    bottom.pwf <- lower.pwf[1] / 2 * fs
    lower.pwf  <- c(bottom.pwf, lower.pwf)
	attributes(lower.pwf)$names <- uni.pres

	## Step 6) Remain original OCO-2 PWF for UPPER OCO-2 levels
    # calculate the PWF for the interface layer between STILT and OCO using SUM(PWF) = 1
	upper.pwf <- oco.pwf[upper.index]
	intf.pwf  <- 1 - sum(lower.pwf) - sum(upper.pwf[-1])

	## Step 7) Calulate the PWF for the very bottom layer
	combine.pwf <- c(lower.pwf, intf.pwf, upper.pwf[-1])
