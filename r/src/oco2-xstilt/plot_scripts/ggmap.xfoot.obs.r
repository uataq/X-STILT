...
ggmap.xfoot.obs <- function(mm, lon.lat, site, oco2.path = NULL, 
  facet.nrow, nhrs, dpar, foot.sf, zisf, met, stilt.ver, timestr, 
  font.size = rel(0.9), recp.lon, recp.lat, foot, foot.sig, 
  titleTF = T, sumTF = T, picname, storeTF = T,
  width = 12, height = 8, anthromesTF = F, anthro.path = NULL){
  ...
  if ('fac' %in% colnames(sel.foot)){
    if (length(unique(sel.foot$fac)) > 1){
      ...
      sum.foot <- sel.foot %>% group_by(fac) %>% 
        dplyr::summarize(sum = sum(foot))
      ...
      p1 <- p1 +
        ...
        geom_text(data = sel.recp, aes(lon + 0.5, lat),
          colour = 'purple', size = 4, label = 'receptor', fontface = 1) +
        facet_wrap(~ fac, nrow = facet.nrow)
    ...
    p1 <- p1 +
      annotate('text',
        x = map.ext$maxlon - (map.ext$maxlon - map.ext$minlon) /10,
  ...
  if (!is.null(oco2.path)) 
    p1 <- p1 + geom_point(data = obs[obs$qf == 0, ], aes(lon, lat, colour = xco2),
      size = 0.3)
  
  p2 <- p1 + 
    geom_raster(data = sel.foot, aes(lon + mm[[3]], lat + mm[[2]], fill = foot),
      alpha = 0.8) +
    scale_fill_gradientn(limits = c(foot.sig, 1E-2), name = 'FOOTPRINT\nppm/(umol/m2/s)',
      trans = 'log10', colours = col,
      breaks = sort(unique(c(foot.sig, 1E-6, 1E-5, 1E-4, 1E-3, 1E-2, 0.1, 1.0))),
      labels = sort(unique(c(foot.sig, 1E-6, 1E-5, 1E-4, 1E-3, 1E-2, 0.1, 1.0)))) +
 ...
 if (!is.null(oco2.path)) {
    ...
    p3 <- p3 + scale_colour_gradientn(name = 'OBS XCO2:', colours = col,
      limits = limits, breaks = breaks, labels = breaks)
 }
  
  p4 <- p3 + guides(colour = guide_colourbar(order = 2),
  ...
  if (anthromesTF) {
    ...
    p4 <- p4 +
      ...
      geom_path(data = f.atm, aes(long, lat, group = group),
        colour = 'darkorange', linetype = 2, size = 1.0)
  }
