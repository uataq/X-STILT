...
ggmap.gpw <- function(lon.lat, pop.path, mm = NULL, site = NULL, picpath = NULL,
                      font.size = rel(1.0), width = 10, height = 11){

  # read data
  library(raster)
  pop.file <- list.files(path = pop.path, pattern = '2015_30_sec.tif')
  pop.dat <- raster(file.path(pop.path, pop.file))

  # crop population data
  sel.pop.dat <- crop(pop.dat, extent(lon.lat$minlon, lon.lat$maxlon, 
                                      lon.lat$minlat, lon.lat$maxlat))
