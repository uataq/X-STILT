...
ggplot.map = function(map = c('black', 'ggmap'), maptype = 'roadmap', 
  ...
  if (map == 'black') {
    ...
    if (lonindx == 0) lonindx = 1
    latrange = seq(minlat, maxlat, ticks[latindx])
    lonrange = seq(minlon, maxlon, ticks[lonindx])
    ...
    m1 = ggplot() + labs(x = 'LONGITUDE', y = 'LATITUDE') +
         geom_polygon(data = worldmap, aes(long, lat, group = group), 
                                           fill = land.col) +
         geom_path(data = usstates, aes(long, lat, group = group), 
                   colour = us.land.outline) +
