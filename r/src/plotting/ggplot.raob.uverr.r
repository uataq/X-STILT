...
ggplot.raob.uverr = function(timestr, site, lon.lat, map = NULL, raob.path, 
    ...
    if (plotTF) {
        ...
        if (is.null(map)) 
            map = ggplot.map(map = 'ggmap', center.lat = lon.lat$site_lat, 
                              center.lon = lon.lat$site_lon, zoom = 5)[[1]]
                         
        r1 = map + #geom_path(data = cir, aes(x, y), linetype = 2, colour = 'gray50') +
            geom_point(data = lon.lat, aes(site_lon, site_lat), size = 5,
    ...
    if (!is.null(err.file)) {
        ...
        err.dat = err.dat %>% 
                   mutate(date = as.POSIXct(as.character(timestr), 
                          ...
                          dist = as.numeric(rdist.earth(
                                            x1 = lon.lat[c('site_lon', 'site_lat')], 
        ...
        if (plotTF) {
            ...
            title = paste0('Time series of u,v errors before overpass time ',
                            timestr, ' for ', site, '\nRMSE = ', 
            ...
            err.png = file.path(err.path, paste0('map_uverr_', site, '_', 
                                                 timestr, '.png'))
