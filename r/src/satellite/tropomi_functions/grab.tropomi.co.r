...
grab.tropomi.co = function(tropomi.path = NULL, timestr = NULL, lon_lat, 
    ...
    h2o_vcd = ncvar_get(dat, 'PRODUCT/SUPPORT_DATA/DETAILED_RESULTS/water_total_column')
    ...
    dimnames(co_vcd_uncert) = dimnames(h2o_vcd) = 
    ...
    sel_df = var_df %>% filter(center_lon >= lon_lat$minlon, 
                               ...
                               center_lat <= lon_lat$maxlat, !is.na(co_vcd)) 
    ...
    merge_df = merge_df %>% 
               mutate(air_vcd = 1 / g / Mair * psfc * 100 - h2o_vcd, 
