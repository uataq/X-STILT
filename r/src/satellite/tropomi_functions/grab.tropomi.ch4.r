...
grab.tropomi.ch4 = function(tropomi.path = NULL, timestr = NULL, lon.lat, 
    ...
    dp   = ncvar_get(dat, 'INPUT_DATA/pressure_interval') / 100  # Pa to hPa
    sfc_class = ncvar_get(dat, 'INPUT_DATA/surface_classification')
    sif  = ncvar_get(dat, 'DETAILED_RESULTS/fluorescence')  # mol s-1 m-2 nm-1 sr-1
    ...
    h2o_weak = ncvar_get(dat, 'INPUT_DATA/water_weak_twoband_total_column')
    h2o_strong = ncvar_get(dat, 'INPUT_DATA/water_strong_twoband_total_column')
    ...
    dimnames(h2o_weak) = dimnames(h2o_strong) = list(indx_across_track, indx_along_track)
    ...
    sel_df = var_df %>% filter(center_lon >= lon.lat$minlon, 
                               ...
                               center_lat <= lon.lat$maxlat, !is.na(ch4_bc)) 
    ...
    if (getakTF) {
        ...
        ak = ncvar_get(dat, 'DETAILED_RESULTS/column_averaging_kernel') 
