...
grab.tropomi.no2 = function(tropomi.path = NULL, timestr = NULL, lon_lat, 
                            tropomi.fn = NULL) {
    ...
    ab_df = data.frame(a_lower = rep(a[1, ], nrow(sel_df)), 
                       ...
                       k = rep(k, nrow(sel_df)), # from sfc to TOA, 0-33
...
# if one provides tropomi.fn, no need to provide tropomi.path and timestr
grab.tropomi.no2.lite = function(tropomi.path = NULL, timestr = NULL, lon_lat, 
                                  tropomi.fn = NULL) {
    ...
    loc_df = loc_df %>% filter(!is.na(lats), lons >= lon_lat$minlon - 0.05, 
                                             lons <= lon_lat$maxlon + 0.05,
                                             lats >= lon_lat$minlat - 0.05, 
                                             lats <= lon_lat$maxlat + 0.05)
