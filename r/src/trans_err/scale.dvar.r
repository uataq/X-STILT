...
scale.dvar <- function(co2.stat){
  ...
  # fit two weighted linear regressions
	...
	fit.lm <- data.frame(
		s = as.numeric(c(fit.lm.pos$coefficients[2], fit.lm.neg$coefficients[2])),
		i = as.numeric(c(fit.lm.pos$coefficients[1], fit.lm.neg$coefficients[1])),
