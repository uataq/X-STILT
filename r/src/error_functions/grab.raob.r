...
grab.raob = function(raob.fn, timestr, err.path, nhrs, 
                     format = c('ncdf', 'fsl')[2], overwriteTF = F){

  filename = file.path(err.path, paste0(site, '_raob_', timestr, '.txt'))
  
  if ( file.exists(filename) & overwriteTF == F ) {
    options(scipen = 999)
    ...
    # ------------------------- if netcdf format ------------------------------
    print(raob.fn)
    if (grepl('tmp', raob.fn)) format = 'fsl'
    if (grepl('nc', raob.fn)) format = 'nc'
