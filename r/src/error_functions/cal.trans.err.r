...
cal.trans.err = function(site, timestr, workdir, traj.path, store.path, met, 
	...
	recp.info = recp.info %>% 
				mutate_all(funs(as.numeric), colnames(recp.info)) %>% 
	...
	for (i in 1 : length(traj.file)) {
		...
		# call cal.var.cov() for vertical-integrated trans error variance
		prod.trans = cal.var.cov(sd = sd.trans[sd.trans < max.sd.trans], 
								  L = Lh, w = w, x = x, type = "ver")	# ppm^2
	...
	cat(paste('cal.trans.err(): receptor-level trans error has been stored in', 
			  txtfile, '...\n'))
