Trajecfoot <- function(ident, part = NULL, timelabel = NULL, pathname = "",
  ...
  for(foottimespos in 1:(length(foottimes)-1)){ #loop over time intervals
    ...
    for (name in unique(emissname)){
      ...
      # debug, DW, 10/16/2017, because our indices (numpix.x, numpix.y) are too large,
      # 1000 is not okay, change to a large number to separate x, and y
      ...
      for(yxg in unique(yxv)){ #for each gridcell...
        ...
        # debug, DW, 10/16/2017, because our indices (numpix) are too large,
        #1000 is not okay, change to 10000 to separate the order
  ...
  # message('rhs')
  # print(str(list(seq(lat.ll,(lat.ll+(numpix.y-1)*lat.res),lat.res),seq(lon.ll,(lon.ll+(numpix.x-1)*lon.res),lon.res),foottimes[1:(length(foottimes)-1)])))
  # message('lhs')
  # print(str(foot.arr))
