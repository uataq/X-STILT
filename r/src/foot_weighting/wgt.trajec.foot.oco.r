...
wgt.trajec.foot.oco <- function(output, oco.path, ak.wgt = T, pwf.wgt = T, 
	...
	if (ak.wgt == FALSE) xstilt.prof <- get.wgt.mod.func(output)
	...
	trajdat.wgt <- trajdat.wgt %>% mutate(foot_wgt = foot * wgt) %>% 
			       dplyr::rename(foot_before_weight = foot, foot = foot_wgt)
...
if (F) {
	...
	x2 <- get.wgt.oco.func(output, oco.path) %>% filter(stiltTF == TRUE)
