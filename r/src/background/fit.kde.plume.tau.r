...
# incorporate typical NOx lifetime into the determination of NO2 plume 
# only for locating sources, not for quantitative calculation

# ------------------------------------------------------------------------------ 
# td for threshold of normalized kernel density
fit.kde.plume.tau = function(site, timestr, traj_path, obs_df, sensor, 
                             td = 0.1, tauTF = T) {

  # ---------------------------------------------------------------------------- 
  # load trajec directories
  traj_dirs = list.dirs(traj_path, recursive = F)
  traj_dir  = traj_dirs[grepl(sensor, traj_dirs) & grepl(timestr, traj_dirs)]
  ...
  trajdat = do.call(rbind, lapply(traj_fns, function(x) {
    ...
    # get receptor info 
    recp_info = ident.to.info(ident = basename(x), stilt.ver = 1, aglTF = F)[[1]]
    datx = readRDS(x) %>% mutate(rel.timestr = recp_info$timestr, 
                                 rel.datestr = as.POSIXlt(rel.timestr, 'UTC', 
                                                          format = '%Y%m%d%H%M'))
    return(datx)
  }))
  ...
  min.datestr = min(trajdat$rel.datestr)
  min.xtime = min(obs_df$datestr) - 1 * 60  # allow for 1 more minutes, convert to sec
  max.xtime = max(obs_df$datestr) + 1 * 60
  sel_traj = trajdat %>% mutate(datestr = min.datestr + time * 60) %>%
  ...
  if (tauTF) {  
    ...
    # before calculating the 2D Kernel density, apply exp decay based on typical lifetime
    # TO DO: think about how to incorporate spatial variation of lifetime 
    tau = 2   # try typical lifetime of 4 hours for wintertime

    # decayed portion = exp(- delt / tau)
    # highest wgt is close to 1 (no decay yet, just released at ~overpass time)
    # lowest wgt is close to 0 (almost decayed for parcels released 12 hours before overpass time)
    # delt is the time diff between overpass time and release time
    # convert unit of `delt` to hours, as tau in hours
    wgt_traj = sel_traj %>% mutate(delt = as.numeric(datestr - rel.datestr) / 60, 
                                   wgt = exp(-delt / tau)) 

    dens = kde2d.wgt(wgt_traj$lon, wgt_traj$lat, h = c(0.05, 0.05), 
                     n = 100, w = wgt_traj$wgt)

    # Normalized KD by the max density
    densw = data.frame(expand.grid(lon = dens$x, lat = dens$y), 
                       prob = as.vector(dens$z)) %>% 
            mutate(norm.prob = prob / max(prob), tau = tau) %>% dplyr::select(-prob) 
  
  } else {  # no lifetime poses
    dens = MASS::kde2d(sel_traj$lon, sel_traj$lat, h = c(0.05, 0.05), n = 100)
  ...
  if (intersectTF) { 
    cat('fit.kde.plume(): found satellite soundings within urban plume :)\n')
                   ...
                   'Likely no valid soundings\n**OR TRY adjusting @param td...\n'))
...
# ---------------------------------------------------------------------------- 
# borrow from https://stat.ethz.ch/pipermail/r-help/2006-June/107405.html
# MASS::kde2d copied and modified
kde2d.wgt = function (x, y, w, h, n = n, lims = c(range(x), range(y))) {
...
# no need to calculate the convex hull...
if (F) {
