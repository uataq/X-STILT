...
ggmap.xfoot.obs <- function(mm, lon.lat, site, oco2.ver, oco2.path = NULL, 
  ...
  if ('fac' %in% colnames(sel.foot)){
    ...
    # summing the values within map domain
    p1 <- p1 + annotate('text', x = map.ext$maxlon - (map.ext$maxlon - map.ext$minlon) /10,
                                y = map.ext$maxlat - (map.ext$maxlat - map.ext$minlat) /10,
                                label = signif(sum(sel.foot$foot), 3), 
                                fontface = 2, size = 5)
  } # end if
  ...
  lab <- 10 ^ seq(-20, 3, 1)
  p2 <- p1 + geom_raster(data = sel.foot, aes(lon + mm[[3]], lat + mm[[2]], fill = foot),
                         alpha = 0.8) +
