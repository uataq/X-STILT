...
ggmap.obs.sif <- function(site, timestr, oco.sensor, sif.path, lon.lat, workdir,
  ...
  title <- paste(oco.sensor, 'SIF [W/m2/sr/µm] for', site, 'on', timestr)
  ...
  d <- rbind(d, d[1,])
  c2 <- c1 + geom_path(data = d, aes(x, y), linetype = 2, size = 0.9, colour = 'gray50') +
