...
mc.lr = function(x, y, x.sd, y.sd, boottime = 5000, reg.model = 'SMA', 
                 posTF = F, printTF = T) {
  ...
  for (n in 1:length(x)) {
    ...
    tmp.norm = data.frame(x = x.norm, y = y.norm, n = rep(n, count), count = seq(1, count))
  ...
  for (c in 1:boottime) {
    ...
    tmp.lm = fit.lm[fit.lm$Method == reg.model, ]
  ...
  if (posTF) stat.all = stat.all %>% filter(slope >= 0)

  if (printTF) cat(paste('mean of r:', signif(mean(stat.all$cor), 3), 
                         '; SD of r:', signif(sd(stat.all$cor), 3), '\n'))
  if (printTF) cat(paste('mean of slope:', signif(mean(stat.all$slope), 3),
                         '; median of slope:', signif(median(stat.all$slope), 3), 
