...
grab_waccm_nox <- function(waccm.fn, xmn, xmx, ymn, ymx, tmn, tmx) {
    ...
    # grab NO in mol / mol, [lon, lat, lev, time]
    no <- ncvar_get(dat, 'NO', start = c(lon.indx[1], lat.indx[1], 1, 1), 
                    count = c(length(lon.indx), length(lat.indx), length(lev), length(nhrs)))
    no2 <- ncvar_get(dat, 'NO2', start = c(lon.indx[1], lat.indx[1], 1, 1), 
                     count = c(length(lon.indx), length(lat.indx), length(lev), length(nhrs)))
    nox = no + no2                     

    dimnames(no) <- dimnames(no2) <- dimnames(nox) <- 
        list(lon[lon.indx], lat[lat.indx], seq(1, length(lev)), nhrs)
    nox_df <- reshape2::melt(nox); colnames(nox_df) <- c('lon', 'lat', 'l', 'hrs', 'nox')
    ...
    # convert to data frame 
    df <- nox_df %>% left_join(ps_df, by = c('lon', 'lat', 'hrs')) %>% 
          left_join(ab_df, by = 'l') %>% 
          mutate(pmid = a * P0 + b * ps, nox_ppb = nox * 1E9, 
                 time = as.POSIXct(hrs * 3600, 'UTC', origin = paste0(waccm.timestr, '00:00:00'))) %>% 
          filter(time >= tmn, time <= tmx)
