...
# DW, 11/29/2022
p_solverv4 = function(pex, mx_res, mx_hr, bg_nox, ts_nox_df, ts_chem = NA, 
                      min_nox = 1e-6) {
    ...
    uni_sza = sort(unique(ts_nox_df$bin_sza))

    if (is.na(mx_res)) {
        mx_res = 5      # in km
        cat(paste('xgas_solver(): use TROPOMI grid spacing as mixing length of', mx_res, 'km...\n'))
    }
    
    p_fill = pex %>% tibble::add_column(p_nox_nochem = NA, p_nox_nomix = NA, 
                                        p_nox_mix = NA, p_co2_nomix = NA, 
                                        p_co2_mix = NA, p_co_nomix = NA, 
                                        p_co_mix = NA, p_ch4_mix = NA, 
                                        p_ch4_nomix = NA, p_rto_nomix = NA, 
    ...
    for (t in 1 : length(times)) {
        ...
        p_init = p_initialv4(p_curr, p_last, mx_res, uni_sza, bg_nox, a_rto = 1)
        #time2 = Sys.time()
        
        # 2 - if MLHT increase over time, perform vertical mixing --------------
        # min_nox as background concentration above mixed layer
        p_ent = p_entrainv4(p_init, delt, dilutionTF = T, min_nox)
        #time3 = Sys.time()

        # 3 - calculate changes in nox due to chem -----------------------------
        p_ts = p_chemicalv3(p_ent, delt, ts_nox_df, OX_df, ts_chem, num_chunk=1)
        
        # double check if there is negative NOx 
        if ( length(p_ts$p_nox_nomix[p_ts$p_nox_nomix < 0]) > 1 ) 
            stop(paste('p_solver3(): capture particles with negative [NOx] for backward time = ', times[t], 'min...\n') )
        #time4 = Sys.time()

        # 4. horizontal mixing of [NOx] among particles
        p_mix = p_mixingv3(p_ts, mx_hr, delt)
        #time5 = Sys.time()
