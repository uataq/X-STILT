...
oco2.get.xco2<-function(ident,sel,site,timestr,ocopath,recp.lat,recp.lon,recp.info,agl.info,
	                      orig.trajpath,new.trajpath,new.intpath,new.ncdfpath,rundir,grdhgt.path,metpath,metfile,nummodel,
	                      ak.weight=T,pw.weight=T,min.lon,max.lon,min.lat,max.lat,foottimes=c(0,72),ct.version="2017",odiac.co2,dmassTF=T,storeTF=F){
  ...
  recp.grdhgt<-get.grdhgt(recp.info=recp.info,nummodel=nummodel,rundir=rundir,grdhgt.path=grdhgt.path,metpath=metpath,metfile=metfile)
