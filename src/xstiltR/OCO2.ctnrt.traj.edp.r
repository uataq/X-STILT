...
endp.time <- weekdayhr(yr=recp.info$recp.year, mon=recp.info$recp.mon,
...
for(f in 1:length(uni.endp.day)){
		...
		ct.timestr <- paste(ct.time["year",],
		                    formatC(ct.time["mon",],  width=2, flag=0),
												...
												formatC(ct.time["sec",],  width=2, flag=0), sep="")
...
ctpattern <- paste(formatC(recp.info$recp.year, width=2, flag=0), "-",
                   formatC(recp.info$recp.mon,  width=2, flag=0), "-",
                   formatC(recp.info$recp.day,  width=2, flag=0), ".nc", sep="")
...
ct.time["hour", ] <- ct.time["hour", ]-1     # by 1 hr
ct.time["min", ]  <- ct.time["min", ] -30    # by 30 mins
