...
# kernel AK, pressure weighting PW, and prior profiles), return 'combine.prof';
# call 'weight.trajecfootv2()' to weight trajec-level footprints using
# interpolated satellite profiles;
...
# 6) call 'ctnrt.background()' to get XCO2 boundary conditions based on
# CarbonTracker global fields;
...
# timestr: time of satellite overpass.
# oco2.path: path that stores OCO-2 Lite files
...
sim.xco2 <- function(namelist, recp.info, odiac.co2){
  ...
  headers <- c('SoundingID', 'recp.lat', 'recp.lon', 'sim.grdhgt',
               paste0('xco2.', namelist$sel), 'obs.xco2', 'sim.xco2', 'diff.xco2')
  ncol <- length(headers)
  ...
  for (i in 1:nrow(recp.info)) {
    ...
    if (!is.na(match('anthro', namelist$sel))) {
      ...
      xco2.anthro <- odiac.anthro(
        foot = foot.anthro, odiac.co2 = odiac.co2, ident = tmp.ident,
        storeTF = namelist$storeTF, ncdfpath = namelist$xco2.path)
    ...
    if (!is.na(match('bio', namelist$sel)) | !is.na(match('ocean', namelist$sel))) {
      ...
      xco2.ct <- ctnrt.bio.ocean(
        ident = tmp.ident, recp.info = tmp.recp.info,
				ct.version = namelist$ct.version, ctpath = namelist$ctflux.path,
				storeTF = namelist$storeTF, foot = foot.ct,
        ncdfpath = namelist$xco2.path)

      xco2.bio <- xco2.ct[1]
      xco2.ocean <- xco2.ct[2]
    ...
    if (!is.na(match('edp', namelist$sel))) {
      ...
      # (AK PW weighted CO2 concentration derived from CT-NRT at endpoints)
			...
			edp.info <- ctnrt.traj.edp(
        ident = tmp.ident, trajdat = wgt.trajdat, recp.info = tmp.recp.info,
        combine.profile = combine.prof, ct.version = namelist$ct.version,
