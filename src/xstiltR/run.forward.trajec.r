...
run.forward.trajec <- function(namelist, plotTF=F){
  ...
  cat(paste("run.forward.trajex(): Generating forward traj at",city.lat,"N\n"))
  agl <- namelist$agl
  nlevel <- length(agl)

  # first releassing time
  ...
  #create traj name
  ident <- paste(rel.yr, "x", formatC(rel.mon, width=2, flag=0), "x",
                              formatC(rel.day, width=2, flag=0), "x",
                              formatC(rel.hr, width=2, flag=0), "x",
                              lat.str, "Nx", lon.str, "Ex",
                              formatC(agl,width=5,flag=0), "x",
                              formatC(npar,width=5,flag=0), "Px",
                              dxyp, "deg", sep="")

  #### if continously release particles
  if(namelist$windowTF){
    ...
    ident <- paste(rel.yr, "x", formatC(rel.mon,width=2,flag=0), "x",
                   ...
                   formatC(npar,width=5,flag=0), "Px", dxyp, "deg",sep="")
  ...
  if(plotTF){
     ...
     pp <- ggplot.forward.trajec(ident=ident, trajpath=namelist$outpath,
                                 ...
                                 ocopath=namelist$ocopath, lat.lon=lat.lon,
                                 zoom=zoom)
