...
timestr = get_timestr(site, lon_lat, obs_sensor, obs_ver, obs_path, store_path, 
                      recp_fn, plotTF = FALSE, qfTF = T, urbanTF = TRUE, 
                      urban_dlon, urban_dlat, sif_path, searchTF = T, 
...
#' # of soundings desired within 1 degree lat range (background or enhanced)
...
# receptors will only be placed based on lati/long info from receptor_demo.csv
...
namelist = list(ak_wgt = ak_wgt, ct_ver = ct_ver, ctflux_path = ctflux_path, 
                ...
                mem_per_node = mem_per_node, met = met,                 met_file_format = met_file_format, met_path = met_path, 
