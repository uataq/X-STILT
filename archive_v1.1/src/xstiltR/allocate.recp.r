...
allocate.recp <- function(namelist){
  ...
  # create new names
  namelist$RUN <- paste0("run_XSTILT_", namelist$timestr)
  namelist$CONTL <- paste0("XSTILT_CONTROL_", namelist$timestr, "_copy")

  # store namelist
  str <- "backward"; if(namelist$forwardTF)str <- "forward"
  namelist$TRAJLIST <- paste0("trajlist_", namelist$site, "_", namelist$timestr,
                       "_", str)
  ...
  if(ncp > 1){
    ...
    one.filenm <- paste0(trajlist, "_cp", ncp, ".rds")
    ...
    cont <- paste0("homedir='", namelist$homedir, "'\nworkdir='",
                  ...
                  "run.backward.trajec(namelist=namelist)")
    cont.filenm <- file.path(namelist$workdir,
      paste0(namelist$CONTL, all.copy, ".r"))
    write(cont, cont.filenm)
  ...
  nodes <- seq(0, ncp - 1, 1)
  r.scripts <- paste0("export script_", nodes, "=", namelist$CONTL, all.copy, ".r")
