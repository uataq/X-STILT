...
weight.trajecfootv2 <- function(ident, recp.info, agl.info,trajdat,recp.grdhgt,
															  oco.ak.norm, oco.pw, oco.pres, oco.apriori,
															  new.trajpath, ak.weight=TRUE, pw.weight=TRUE){

each.par <- recp.info$each.par

# HERE, ak.weight and pw.weight is passed on for weighting trajec
combine.profile <- get.weight.funcv2(ident=ident, recp.info=recp.info,
	                                   agl.info=agl.info, trajdat=trajdat,
																		 recp.grdhgt=recp.grdhgt,
																	   oco.ak.norm=oco.ak.norm, oco.pw=oco.pw,
																		 oco.pres=oco.pres, oco.apriori=oco.apriori,
																	   ak.weight=ak.weight, pw.weight=pw.weight)

### STARTing weighting trajec based on profiles
if(ak.weight==F & pw.weight==F){

		# if ak.weight==F && pw.weight==F, return trajec with original footprint,
		# no longer need any following weighting...
		# !!! but still need to return weighting functions and other info
		cat("weight.trajecfootv2(): no wgt, return profiles and original trajec\n")
	...
	# weighting newfoot by multipling AK and PW profiles from "combine.profile",
	...
	if(ak.weight==T & pw.weight==T){

    # weighted by AK*PW and total STILT levels !!!
		cat("weight.trajecfoot(): weight trajec by both AK & PW profiles\n")
		for (l in 1:stilt.nlevel){
			trajdat[which(trajdat[,"level"]==l),"newfoot"]<-trajdat[which(trajdat[,"level"]==l),"foot"] * stilt.profile$ak.pw[l] * stilt.nlevel
		}

	}else if(ak.weight==F & pw.weight==T){

		# weighted by PW and total STILT levels !!!
		cat("weight.trajecfoot(): weight trajec only by PW profiles\n")
		for (l in 1:stilt.nlevel){
			trajdat[which(trajdat[,"level"]==l),"newfoot"]<-trajdat[which(trajdat[,"level"]==l),"foot"] * stilt.profile$pw[l] * stilt.nlevel
		}

	}else if(ak.weight==T & pw.weight==F){

		# weighted by only AK, for calculating dCO2 for each trajec in doing transport error !!!
		cat("weight.trajecfoot(): weight trajec only by AK profiles\n")
		for (l in 1:stilt.nlevel){
			trajdat[which(trajdat[,"level"]==l),"newfoot"]<-trajdat[which(trajdat[,"level"]==l),"foot"] * stilt.profile$ak.norm[l]
		}
