...
get.foot <- function(ident, foot.overwrite = F, trajdat, trajpath, footpath,
                     foottimes, zlim = c(0, 0), dmassTF, coarse = 1,
                     fluxweighting = NULL, numpix.x, numpix.y, storeTF = F,
  ...
  intfoot.name <- paste0("intfoot", ident, ".nc")

  # only for uneven ident, change "&" to "+"
  if (grepl("&", ident)){
    ident2 <- gsub("&", "+", ident)
    intfoot.name <- paste0("intfoot", ident2, ".nc")
  }
  ...
  if (foot.overwrite == TRUE | find.flag == FALSE){
	  ...
	  foot <- Trajecfoot(
			ident = ident, part = trajdat, pathname = trajpath, zlim = zlim,
    ...
    # if need to store time-integrated footprint
      ...
      if (storeTF) {
        ...
        foot.var <- ncvar_def(
					name = "footprint", units = "PPM/(umoles/m2/s)", list(y, x),
			    longname = "time-integrated 2D footprint")
