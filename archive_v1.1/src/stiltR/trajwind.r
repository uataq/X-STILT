...
info<-Trajec(nturb=T,yr=yr,mon=mon,day=day,hr=hr,lat=lat,lon=lon,agl=agl,nhrs=nhrs,numpar=1,outdt=1,delt=1,
             metlib=metlib,metfile=metfile,metd=metd,doublefiles=doublefiles,overwrite=T,
             rundir=rundir,nummodel=nummodel,varsout=varsout,outpath=outpath,outname=outname,...) #pass on the outpath to Trajec.r
...
sel<-abs(tdat[,"time"])==min(unique(abs(tdat[,"time"])))	# select the min timestep, which is the most closed to the receptor
