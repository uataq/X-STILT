...
if(debugTF){
  ...
  timelabel=NULL
  dmassTF=T
  vegpath="/deas/group/stilt/Vegetation/"
  wrfinput=NULL
}
...
#
# 01/16/2017, edited by DW, change .RDataxname to xname.RData
#  $Id: Trajecfoot.r,v 1.17 2009/02/11 13:03:13 gerbig Exp $

# fix by DW, 10/16/2017, fixing weird shift in longitude
#---------------------------------------------------------------------------------------------------

# debug, DW, 10/16/2017
#sel.part<-part[part[,"lon"]>6&part[,"lon"]<6.1,]
#sel.part<-sel.part[sel.part[,"foot"]>0,]

#JCL:(04/19/2004) enable capability to directly pass on the object
if(is.null(part)){   #when object wasn't passed on
  #Check if object exists
  # Edited by DW--01/18/2017
  #if(length(unix.shell(paste("if (-e ",pathname,ident,".RData) echo 'found it!'; endif",sep=""),shell="/bin/csh"))>0){
  trajfile<-list.files(pattern=ident, path=pathname,all.files=TRUE)
  if(length(trajfile)>0){
    ...
    print(paste("Trajecfoot(): object=",pathname,ident,".RData NOT FOUND",sep=""))
    return()
  } #if exists or not
} #if(!is.null(part)){
...
for(foottimespos in 1:(length(foottimes)-1)){ #loop over time intervals
  ...
  # debug, DW, 10/16/2017
  #sel.part<-subpart[subpart[,"lon"]>6&subpart[,"lon"]<6.1,] # subpart has already been filtered by zero footprint, should be no values in sel.part
...
coarsex<-c(1,1,2,2,2,4,4,4,8,8,8,16,16,16,32,32)  #factors by which grids have been made coarser
coarsey<-c(1,2,1,2,4,2,4,8,4,8,16,8,16,32,16,32)  #e.g., '4' means grid is 4 times coarser
#coarsex<-rep(1,16)  #factors by which grids have been made coarser
#coarsey<-rep(1,16)  #e.g., '4' means grid is 4 times coarser
...
for (name in unique(emissname)){
  ...
  # DW, 10/16/2017, name==emissname[1] is the most near-field block
  sel.part<-subpart[sel,]
  ...
  # debug, DW, 10/16/2017, because our indices (numpix.x, numpix.y) are too large, 1000 is not okay, change to a large number to separate x, and y
	#yxv<-y*1000+x #separate in order of magnitude
  yxv<-y*100000+x #separate in order of magnitude
  ...
  if(zlim[2]==0){ #surface influence
    ...
    # debug, DW, 10/16/2017, because our indices (numpix) are too large, 1000 is not okay, change to 10000 to separate the order
		#xfine<-yxg-floor(yxg/1000)*1000
		#yfine<-floor(yxg/1000)
    xfine<-yxg-floor(yxg/100000)*100000
    yfine<-floor(yxg/100000)
    ...
    npixfx<-length(xfine)
    npixfy<-length(yfine)
    # !!! line for calculating 2D footprint in each gridcell...
