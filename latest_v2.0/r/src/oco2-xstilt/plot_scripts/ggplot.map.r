...
ggplot.map <- function(map = c("black","ggmap"), maptype = 'roadmap',
                       ...
                       shape.path = "/uufs/chpc.utah.edu/common/home/lin-group1/wde",
  ...
  if(map == "black"){
    ...
    shape.file <- file.path(shape.path, shape.file)
    ...
    m1 <- ggplot() +
          geom_polygon(worldmap, aes(long, lat, group), fill = land.col) +
          geom_path(worldmap, aes(long, lat, group), colour = land.outline) +
